...
def make_task(id=1, goal_id=1, status="pending", notes=None):
    t = MagicMock()
    t.id = id; t.goal_id = goal_id; t.status = status; t.notes = notes
    return t


def make_goal(id=1, user_id=1):
...
def test_RED_notes_are_stripped_of_whitespace():
    ...
    task = make_task(id=1); goal = make_goal(user_id=1)
    db = MagicMock()
    db.query.return_value.filter.return_value.first.side_effect = [task, goal]
    update_task_notes(
        task_id=1, body=TaskNotesUpdate(notes="   my notes   "),
        current_user={"user_id": 1}, db=db
    )
...
def test_RED_batch_invalid_status_raises_400():
    ...
    with pytest.raises(HTTPException) as exc:
        batch_update_tasks(
            body=TaskBatchUpdate(task_ids=[1], status="wrong"),
            current_user={"user_id": 1}, db=db
        )
...
def test_GREEN_notes_db_commit_and_refresh_called():
    ...
    from routers.task_routes import update_task_notes, TaskNotesUpdate
    task = make_task(id=1); task.notes = "note"
    goal = make_goal(user_id=1)
    db = MagicMock()
    db.query.return_value.filter.return_value.first.side_effect = [task, goal]
    update_task_notes(
        task_id=1, body=TaskNotesUpdate(notes="note"),
        current_user={"user_id": 1}, db=db
    )
...
def test_GREEN_batch_returns_updated_ids_and_status():
    ...
    from routers.task_routes import batch_update_tasks, TaskBatchUpdate
    task1 = make_task(id=1); task2 = make_task(id=2)
    goal = make_goal(user_id=1)
    db = MagicMock()
    db.query.return_value.filter.return_value.first.side_effect = [task1, goal, task2, goal]
    result = batch_update_tasks(
        body=TaskBatchUpdate(task_ids=[1, 2], status="completed"),
        current_user={"user_id": 1}, db=db
    )
    assert set(result["updated_task_ids"]) == {1, 2}
...
def test_GREEN_batch_skips_unauthorized_tasks_silently():
    ...
    from routers.task_routes import batch_update_tasks, TaskBatchUpdate
    task1 = make_task(id=1); goal = make_goal(user_id=1)
    db = MagicMock()
    ...
    assert 1 in result["updated_task_ids"]
    assert 2 not in result["updated_task_ids"]
...
def test_REFACTOR_batch_uses_single_commit_for_all_tasks():
    ...
    db.query.return_value.filter.return_value.first.side_effect = [task1, goal, task2, goal]
    batch_update_tasks(
        body=TaskBatchUpdate(task_ids=[1, 2], status="missed"),
        current_user={"user_id": 1}, db=db
    )
    db.commit.assert_called_once()