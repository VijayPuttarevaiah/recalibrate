...
def make_task(id=1, goal_id=1, status="pending"):
    t = MagicMock()
...
def test_RED_valid_statuses_set_has_four_members():
    ...
    from routers.task_routes import VALID_STATUSES
...
def test_RED_invalid_status_raises_400():
    ...
    from routers.task_routes import update_task_status, TaskStatusUpdate
    db = MagicMock()
    with pytest.raises(HTTPException) as exc:
        update_task_status(
            task_id=1, body=TaskStatusUpdate(status="done"),
...
def test_GREEN_response_has_task_id_and_new_status():
    ...
    from routers.task_routes import update_task_status, TaskStatusUpdate
    task = make_task(id=5, status="pending")
    goal = make_goal(user_id=1)
    db = MagicMock()
    db.query.return_value.filter.return_value.first.side_effect = [task, goal]
    result = update_task_status(
        task_id=5, body=TaskStatusUpdate(status="missed"),
        current_user={"user_id": 1}, db=db
    )
...
def test_REFACTOR_get_user_task_raises_404_for_missing_task():
    ...
    from routers.task_routes import _get_user_task
    db = MagicMock()
    db.query.return_value.filter.return_value.first.return_value = None
    with pytest.raises(HTTPException) as exc:
        _get_user_task(db, task_id=999, user_id=1)
...
def test_REFACTOR_get_user_task_raises_403_for_wrong_user():
    ...
    with pytest.raises(HTTPException) as exc:
        _get_user_task(db, task_id=1, user_id=99)
...
def test_REFACTOR_get_user_task_returns_task_for_valid_owner():
    ...
    from routers.task_routes import _get_user_task
    task = make_task(id=1, goal_id=1)
    goal = make_goal(user_id=1)
    db = MagicMock()
    db.query.return_value.filter.return_value.first.side_effect = [task, goal]
    result = _get_user_task(db, task_id=1, user_id=1)
    assert result.id == 1