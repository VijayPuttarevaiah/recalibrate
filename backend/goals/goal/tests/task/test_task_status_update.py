...
def test_refactor_task_missing_404():
    """get_user_task raises 404 when task doesn't exist."""
    from goals.routers.task_routes import get_user_task
    ...
    with pytest.raises(HTTPException) as exc:
        get_user_task(db, task_id=999, user_id=DEFAULT_USER_ID)
    assert exc.value.status_code == HTTP_NOT_FOUND


def test_refactor_wrong_user_403():
    """get_user_task raises 403 when task belongs to another user."""
    from goals.routers.task_routes import get_user_task
    ...
    with pytest.raises(HTTPException) as exc:
        get_user_task(db, task_id=DEFAULT_TASK_ID, user_id=OTHER_USER_ID)
    assert exc.value.status_code == HTTP_FORBIDDEN


def test_refactor_task_valid_owner():
    """get_user_task returns task when ownership confirmed."""
    from goals.routers.task_routes import get_user_task
    ...
    filtered = query.filter.return_value
    filtered.first.side_effect = [task, goal]
    result = get_user_task(db, task_id=DEFAULT_TASK_ID, user_id=DEFAULT_USER_ID)
