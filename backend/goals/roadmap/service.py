...
CODING_ROADMAPS = {
    "beginner": [
        ...
        {
            "phase": "Month 3",
            "steps": ["Build 2 portfolio projects"],
        },
        {
            "phase": "Month 4",
            "steps": [
...
class RoadmapService:
    ...
    def _is_fitness(self, interest: str) -> bool:
        return any(
            k in interest for k in ["fitness", "health", "weight", "sport", "gym"]
        )

    def _is_immigration(self, interest: str) -> bool:
        return any(
            k in interest
            for k in ["immigration", "visa", "pr", "passport", "residency"]
        )

    # ── Coding agent ───────────────────────────────────────────────────────────

    def _coding_roadmap(self, experience_level: str) -> list[dict]:
        return CODING_ROADMAPS.get(experience_level, CODING_ROADMAPS["advanced"])

    # ── Fitness agent ──────────────────────────────────────────────────────────

    def _fitness_roadmap(self, experience_level: str) -> list[dict]:
        return FITNESS_ROADMAPS.get(experience_level, FITNESS_ROADMAPS["advanced"])
    ...
    def _career_roadmap(self, experience_level: str) -> list[dict]:
        return CAREER_ROADMAPS.get(experience_level, CAREER_ROADMAPS["advanced"])
