...
def test_counts_missed_tasks():
    ...
    tasks = [
        ...
        make_task(
            ...
            "Future",
            "pending",
            today + timedelta(days=FUTURE_DAYS + OVERDUE_DAYS_LARGE),
        ),
    ]
    result = build_progress_summary(
        make_db(tasks), goal_id=DEFAULT_GOAL_ID, as_of=today
...
def test_green_completion_mixed():
    ...
    tasks = [
        make_task(
            ...
            "Done 1",
            "completed",
            today - timedelta(days=OVERDUE_DAYS_XL),
        ),
        make_task(
            SECOND_TASK_ID,
            DEFAULT_GOAL_ID,
...
def test_green_phase_keys_format():
    ...
    tasks = [
        make_task(
            ...
            "Old",
            "completed",
            today - timedelta(days=PHASE_DAYS),
        )
    ]
    result = build_progress_summary(
        make_db(tasks), goal_id=DEFAULT_GOAL_ID, as_of=today
...
def test_refactor_missed_capped():
    ...
    missed = [
        {"title": f"Missed Task {i}", "due_date": "2026-01-01"}
        for i in range(MISSED_TASKS_TOTAL)
    ]
    summary = {
        "stats": {
            "completed": 0,
            "missed": MISSED_TASKS_TOTAL,
            "total_tasks": MISSED_TASKS_TOTAL,
            "completion_rate": 0.0,
        },
        "missed_tasks": missed,
        "recent_completed": [],
...
def test_refactor_output_budget():
    ...
    missed = [
        ...
        for i in range(MISSED_TASKS_LARGE)
    ]
    summary = {
        "stats": {
            "completed": SUMMARY_COMPLETED,
            "missed": MISSED_TASKS_LARGE,
            "total_tasks": SUMMARY_TOTAL,
            "completion_rate": COMPLETION_RATE_SMALL,
        },
        "missed_tasks": missed,
